...
import com.loc.newsapp.data.remote.CalendarNewsPagingSource
...
class NewsRepositoryImpl(private val newsApi: NewsApi, private val newsDao: NewsDao) :
    ...
    override fun calendarNews(from: String, sources: List<String>): Flow<PagingData<Article>> {
        return Pager(
            config = PagingConfig(pageSize = 10),
            pagingSourceFactory = {
                CalendarNewsPagingSource(
                    from = from,
                    newsApi = newsApi,
                    sources = sources.joinToString(separator = ",")
                )
